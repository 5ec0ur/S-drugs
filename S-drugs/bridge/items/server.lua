...
function it.hasItem(source, item, amount, metadata)
    ...
    lib.print.error('[' ..it.name..' | hasItem] Need more information about this error? Look here https://help.S-scripts.com/errors/inventory')
    return false
end
...
function it.giveItem(source, item, amount, metadata)
    ...
    lib.print.error('[bridge | giveItem] Need more information about this error? Look here https://help.S-scripts.com/errors/inventory')
    return false
end

lib.callback.register('S-drugs:server:giveItem', function(source, item, amount, metadata)
...
function it.removeItem(source, item, amount, metadata)
    ...
    lib.print.error('[bridge | removeItem] Need more information about this error? Look here https://help.S-scripts.com/errors/inventory')
    return false
end

lib.callback.register('S-drugs:server:removeItem', function(source, item, amount, metadata)
...
function it.createUsableItem(item, cb)
    if ConsumableItems[item] then print('[S-lib] The item ' .. item .. ' is already registered as a consumable item. Skipping the registration of this item.') end
...
function it.getItemLabel(source, itemName)
    local itemLabel
    if it.inventory == 'ox' then
        itemLabel = lib.callback.await('S-lib:client:getItemLabel', source, itemName)
    end
...
lib.callback.register('S-lib:server:getItemLabel', function(source, itemName)
    local itemLabel = it.getItemLabel(source, itemName)
    return itemLabel
end)

lib.callback.register('S-lib:hasItem', function(source, item, amount)
    local hasItem = it.hasItem(source, item, amount)
    return hasItem
end)

lib.callback.register('S-lib:getItemCount', function(source, item)
    local itemCount = it.getItemCount(source, item)
    return itemCount
end)

RegisterNetEvent('S-lib:toggleItem', function(toggle, name, amount, metadata)
    local source = source
