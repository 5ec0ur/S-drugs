...
function it.getCitizenId()
    local citizenId = lib.callback.await('S-lib:getCitizenId', false)
    return citizenId
end
...
function it.getPlayerName()
    ...
    elseif it.core == 'esx' then
        return lib.callback.await('S-lib:getPlayerName', false)
    end
end

function it.getLicences()
    local licences = lib.callback.await('S-lib:getLicences', false)
    return licences
end

function it.getLicence(licenseType)
    local licence = lib.callback.await('S-lib:getLicence', false, licenseType)
    return licence
end