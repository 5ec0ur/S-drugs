...
local function createPlantTargets()
                    ...
                    options = {
                        {
                            ...
                            action = function (entity)
                                ...
                                lib.callback("S-drugs:server:getPlantByNetId", false, function(plantData)
                                    if not plantData then
                                        lib.print.error('[S-drugs] Unable to get plant data by network id')
                                    else
                                        if Config.Debug then
                                            lib.print.info('[plantSelect] Current plant data: ', plantData)
                                        end
                                        TriggerEvent('S-drugs:client:showPlantMenu', plantData)
                                    end
                                end, networkId)
                            end
                        }
    ...
    elseif targetSystem == 'ox_target' then
        for k, v in pairs(Config.PlantTypes) do
            for _, plant in pairs(v) do
                exports.ox_target:removeModel(plant[1], 'S-drugs-check-plant')
                exports.ox_target:addModel(plant[1], {
                    {
                        label = _U('TARGET__PLANT__LABEL'),
                        name = 'S-drugs-check-plant',
                        icon = 'fas fa-eye',
                    ...
                    options = {
                        {
                            ...
                            action = function (entity)
                                TriggerEvent('S-drugs:client:showDealerActionMenu', k)
                            end
                        }
                    },
                    distance = 1.5,
                    ...
                    options = {
                        {
                            ...
                            action = function (entity)
                                local networkId = NetworkGetNetworkIdFromEntity(entity)
                                lib.callback("S-drugs:server:getTableByNetId", false, function(tableData)
                                    if not tableData then
                                        lib.print.error('[S-drugs] Unable to get table data by network id')
                                    else
                                        if Config.Debug then
                                            lib.print.info('[createProccessingTargets] Current table data: ', tableData)
                                        end
                                        TriggerEvent('S-drugs:client:showRecipesMenu', {tableId = tableData.id})
                                    end
                                end, networkId)
                            end
                        }
            ...
            options = {
                {
                    ...
                    icon = 'fas fa-comment',
                    action = function(entity)
                        TriggerEvent('S-drugs:client:checkSellOffer', entity)
                    end,
        ...
        exports.ox_target:addGlobalPed({
            {
                ...
                icon = 'fas fa-comment',
                onSelect = function(data)
                    TriggerEvent('S-drugs:client:checkSellOffer', data.entity)
                end,
...
RemoveSellTarget = function()
    ...
    elseif targetSystem == 'ox_target' then
        ...
        exports.ox_target:removeGlobalPed('S-drugs-sell')
    end
end
    ...
    elseif targetSystem == 'ox_target' then
        ...
        if Config.EnableProcessing then
            for _, v in pairs(Config.ProcessingTables) do
                if v.model ~= nil then
                    exports.ox_target:removeModel(v.model, 'S-drugs-use-table')
                end
            end
        end
    end
