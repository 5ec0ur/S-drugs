...
local function plantSeed(ped, plant, plantInfos, plantItem, coords, metadata)
    ...
    local plants = lib.callback.await('S-drugs:server:getPlants', false)
    ...
    if ShowProgressBar({
        ...
        TriggerServerEvent('S-drugs:server:createNewPlant', coords, plantItem, zone, metadata)
        ClearPedTasks(ped)
...
-- Events
RegisterNetEvent('S-drugs:client:useSeed', function(plantItem, metadata)
    ...
    local ownedPlants = lib.callback.await('S-drugs:server:getPlantByOwner', false)
...
RegisterNetEvent('S-drugs:client:harvestPlant', function(args)
    ...
    if ShowProgressBar({
        ...
        TriggerServerEvent('S-drugs:server:harvestPlant', plantData.id)
        ClearPedTasks(ped)
        RemoveAnimDict('amb@medic@standing@kneel@base')
        RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
    else
...
local giveWater = function(args)
    ...
    if ShowProgressBar({
        ...
        TriggerServerEvent('S-drugs:server:plantTakeCare', plantData.id, item)
        ClearPedTasks(ped)
...
RegisterNetEvent('S-drugs:client:useItem', function (args)
    local item = args.item
...
RegisterNetEvent('S-drugs:client:destroyPlant', function(args)
    ...
    if ShowProgressBar({
        ...
        TriggerServerEvent('S-drugs:server:destroyPlant', {plantId = plantData.id})
        ClearPedTasks(ped)
        RemoveAnimDict('amb@medic@standing@kneel@base')
        RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
    else
...
RegisterNetEvent('S-drugs:client:startPlantFire', function(coords)
    local ped = PlayerPedId()
