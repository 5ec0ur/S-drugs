...
RegisterNetEvent('S-drugs:client:syncRestLoop', function(status)
    restLoop = status
end)

RegisterNetEvent('S-drugs:client:syncPlants', function(plantList)
    if Config.Debug then lib.print.info('Syncing plants', plantList) end
    plants = plantList
end)

RegisterNetEvent('S-drugs:client:syncTables', function(tableList)
...
CreateThread(function()
    ...
    plants = lib.callback.await('S-drugs:server:getPlants', false)
    processingTables = lib.callback.await('S-drugs:server:getTables', false)
    dealers = lib.callback.await('S-drugs:server:getDealers', false)
        ...
        if not restLoop then
            ...
            if closestDealer and dealerDistance <= 2.0 then
                ...
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('S-drugs:client:showDealerActionMenu', closestDealer.id)
                end
            else
                if plantDistance > tableDistance then
                    if closestTable and tableDistance <= 2.0 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            TriggerEvent('S-drugs:client:showRecipesMenu', {tableId = closestTable.id})
                        end
                    end
                elseif closestPlant and plantDistance <= 2.0 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent('S-drugs:client:showPlantMenu', closestPlant)
                    end
                end
            end
        end
...
CreateThread(function()
    while true do
        ...
        local newPlants = lib.callback.await('S-drugs:server:getPlants', false)
        if newPlants then
            plants = newPlants
        end

        local newProcessingTables = lib.callback.await('S-drugs:server:getTables', false)
