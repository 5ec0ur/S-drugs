...
local placeProcessingTable = function(ped, tableItem, coords, rotation, metadata)
    ...
    if lib.progressBar({
        ...
        TriggerServerEvent('S-drugs:server:createNewTable', coords, tableItem, rotation, metadata)
...
RegisterNetEvent('S-drugs:client:placeProcessingTable', function(tableItem, metadata)
...
RegisterNetEvent('S-drugs:client:processDrugs', function(args)
    
    local tableData = lib.callback.await('S-drugs:server:getTableById', false, args.tableId)
    local recipe = lib.callback.await('S-drugs:server:getRecipeById', false, args.tableId, args.recipeId)
    ...
    if recipe.particlefx then
        ...
        TriggerServerEvent("S-drugs:server:syncparticlefx", true, tableData.id, tableData.netId, recipe.particlefx)
    end
    ...
    if Config.ProcessingSkillCheck then
        for i = 1, amount do
            ...
            if success then
                ...
                TriggerServerEvent('S-drugs:server:processDrugs', {tableId = args.tableId, recipeId = args.recipeId})
            else
                proccessing = false
    ...
    if recipe.particlefx then
        ...
        TriggerServerEvent("S-drugs:server:syncparticlefx", false, tableData.id, nil, nil)
    end
    TriggerEvent('S-drugs:client:syncRestLoop', false)
end)

RegisterNetEvent('S-drugs:client:removeTable', function(args)
    ...
    if ShowProgressBar({
        ...
        TriggerServerEvent('S-drugs:server:removeTable', {tableId = args.tableId})
        ClearPedTasks(ped)
        RemoveAnimDict('amb@medic@standing@kneel@base')
        RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
    else
...
RegisterNetEvent('S-drugs:client:syncparticlefx', function(status, tableId, netId, particlefx)
    if status then
