...
RegisterNetEvent('S-drugs:client:checkSellOffer', function(entity)
	local copsAmount = lib.callback.await('S-drugs:server:getCopsAmount', false)
	...
	TriggerEvent('S-drugs:client:showSellMenu', {item = sellItemData.item, price = sellItemData.price, amount = sellAmount, entity = entity})
	SetTimeout(Config.SellSettings['sellTimeout']*1000, function()
		...
		if lib.getOpenContextMenu() ~= nil then
			...
			if currentMenu == 'S-drugs-sell-menu' then
...
RegisterNetEvent('S-drugs:client:salesInitiate', function(cad)
	AddSoldPed(cad.tped)
	if cad.type == 'close' then
		...
		TriggerServerEvent('S-drugs:server:initiatedrug', cad)
		SetPedAsNoLongerNeeded(cad.tped)
	end
end)
