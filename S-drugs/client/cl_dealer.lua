...
CreateThread(function()
    for dealerID, dealerData in pairs(Config.DrugDealers) do
        ...
        local dealerPosition = lib.callback.await('S-drugs:server:getDealerPosition', false, dealerID)

        while not dealerPosition do
            Wait(500)
            dealerPosition = lib.callback.await('S-drugs:server:getDealerPosition', false, dealerID)
...
RegisterNetEvent('S-drugs:client:handleDealerInteraction', function(args)
    ...
    if action == 'sell' then
        TriggerServerEvent('S-drugs:server:sellItemsToDealer', dealerId, item, amount, total)
        return
    end

    if action == 'buy' then
        TriggerServerEvent('S-drugs:server:buyItemsFromDealer', dealerId, item, amount, total)
        return
    end
end)
